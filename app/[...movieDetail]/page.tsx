            ...
            {selectedMovie && (
                <div key={selectedMovie.id} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    <div className="flex flex-col col-span-2">
                        <img className="w-full rounded h-auto mb-4" src={`https://www.themoviedb.org/t/p/w500_and_h282_face${selectedMovie.poster_path}`} alt={selectedMovie.title} />
                        <h2 className="text-xl font-semibold mb-4">{selectedMovie.title}</h2>
                        <p className="text-white text-m">{selectedMovie.overview}</p>
